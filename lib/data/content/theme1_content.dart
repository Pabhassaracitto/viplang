...
Trong văn phòng, các tình huống sẽ là về các vấn đề gồm họp hành, thông báo nội bộ, thư tín, bản fax, thư điện tử, các phòng ban, tất cả các loại thiết bị văn phòng mà cần được quan tâm như là máy phô-tô, máy fax, máy tính, máy điều hòa, hay điện thoại. Chuyên đề Văn phòng này cũng sử dụng các thông báo bằng lời và các thông báo nội bộ dưới dạng viết để truyền tải những tin tức quan trọng tới nhân viên.''',
  ...
  static List<MixedSegment> get _day1MindGameSegments => [
    ...
    const MixedSegment.english(', telephones. It is helpful to understand '),
    ...
    const MixedSegment.english(
      ' a meeting\', \'to downsize a department\', or idiomatic/slang ',
    ),
    ...
    const MixedSegment.english(
      '\' or \'let someone go\' all of which mean to ',
    ),
    const MixedSegment.vietnamese('chấm dứt', 'terminate'),
    const MixedSegment.english(' (end) someone\'s employment. '),
    const MixedSegment.vietnamese(
      'Để nhắc lại vấn đề đã nêu',
      'To repeat an earlier point',
    ),
    const MixedSegment.english(', single words and their '),
    const MixedSegment.vietnamese('các từ đồng nghĩa', 'synonyms'),
    const MixedSegment.english(
      ' are not enough to do well on the TOEIC test, ',
    ),
    const MixedSegment.vietnamese(
    ...
    const MixedSegment.english(
      ' in conversations of an informal nature. The office theme also uses ',
    ),
    const MixedSegment.vietnamese(
